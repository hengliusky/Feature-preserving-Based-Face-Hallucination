...
def main():
    ...
    tl.files.exists_or_mkdir(checkpoint_dir + '/gan')
    tl.files.exists_or_mkdir(summary_dir + '/gan')
    ...
    loss_writer = tf.summary.FileWriter(summary_dir + '/gan', sess.graph)
    sess.run(tf.global_variables_initializer())
    if tl.files.load_and_assign_npz(
            sess=sess, name=checkpoint_dir + '/gan/generator.npz',
    ...
    tl.files.load_and_assign_npz(
        sess=sess,
        name=checkpoint_dir + '/gan/discriminator.npz',
        network=net_d)
    ...
    for epoch in range(num_epoch):
        for idx in range(0, len(train_hr_imgs), batch_size):
            ...
            ## update G
            errG, mse, vgg, g_gan, _ = sess.run(
                [g_loss, mse_loss, vgg_loss, g_gan_loss, g_optim], {
                    t_image: lr_image,
                    t_target_image: hr_image
                })
            print(
                "Epoch [%2d/%2d] %4d, d_loss: %.8f g_loss: %.8f(mse=%.8f, vgg=%.8f, g_gan=%.8f)  time: %.4f"
                % (epoch + 1, num_epoch, step, errD, errG, mse, vgg, g_gan,
                   time.time() - start_time))
            step = step + 1

        tl.files.save_npz(
            net_g.all_params,
            name=checkpoint_dir + '/gan/generator.npz',
            sess=sess)
        tl.files.save_npz(
            net_d.all_params,
            name=checkpoint_dir + '/gan/discriminator.npz',
        ...
        tl.files.save_npz(
            net_g.all_params,
            name=checkpoint_dir +
            '/gan/generator_%d_%d.npz' % (epoch + 1, step),
            sess=sess)
        tl.files.save_npz(
            net_d.all_params,
            name=checkpoint_dir +
            '/gan/discriminator_%d_%d.npz' % (epoch + 1, step),
            sess=sess)
