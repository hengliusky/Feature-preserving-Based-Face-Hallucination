...
from config import *
from model import Vgg19_simple_api, discriminator, encoder, generator, code_discriminator
from utils import *
...
def main():
    ...
    tl.files.exists_or_mkdir(checkpoint_dir + '/full')
    tl.files.exists_or_mkdir(summary_dir + '/full')
    ...
    net_c, logits_code_real = code_discriminator(
        enc_target_emb, is_train=True, reuse=False)
    _, logits_code_fake = code_discriminator(
        enc_predict_emb, is_train=True, reuse=True)
    ...
    ###=============================GAN LOSS=============================###
    d_gan_loss = -1e-3 * tf.reduce_mean(
    ...
    ###============================CODER LOSS============================###
    d_code_loss = -1e-3 * tf.reduce_mean(
        tf.log(tf.nn.sigmoid(logits_code_real - logits_code_fake) + 1e-9))
    g_code_loss = -1e-3 * tf.reduce_mean(
        tf.log(tf.nn.sigmoid(logits_code_fake - logits_code_real) + 1e-9))
    ###==================================================================###

    g_loss = mse_loss + g_gan_loss + vgg_loss + g_code_loss
    d_loss = d_gan_loss + d_code_loss
    ...
    with tf.name_scope('summary'):
        ...
        tf.summary.scalar('d_gan_loss', d_gan_loss)
        tf.summary.scalar('g_code_loss', g_code_loss)
        tf.summary.scalar('d_code_loss', d_code_loss)

    c_vars = tl.layers.get_variables_with_name('code_discriminator', True,
                                               True)
    ...
    g_optim = tf.train.AdamOptimizer(
        2e-4, beta1=0.5).minimize(
            g_loss, var_list=[g_vars, e_vars])
    d_optim = tf.train.AdamOptimizer(
        2e-4, beta1=0.5).minimize(
            d_loss, var_list=[d_vars, c_vars])

    ###========================== RESTORE MODEL =============================###
    sess = tf.Session(
        config=tf.ConfigProto(
            allow_soft_placement=True, log_device_placement=False))
    loss_writer = tf.summary.FileWriter(summary_dir + '/full', sess.graph)
    sess.run(tf.global_variables_initializer())
    if tl.files.load_and_assign_npz(
            sess=sess, name=checkpoint_dir + '/full/generator.npz',
    ...
    tl.files.load_and_assign_npz(
        sess=sess,
        name=checkpoint_dir + '/full/discriminator.npz',
        network=net_d)
    tl.files.load_and_assign_npz(
        sess=sess,
        name=checkpoint_dir + '/full/code_discriminator.npz',
        network=net_c)
    if tl.files.load_and_assign_npz(
        sess=sess, name=checkpoint_dir + '/full/encoder.npz', network=net_e) is False:
        tl.files.load_and_assign_npz(
    ...
    for epoch in range(num_epoch):
        for idx in range(0, len(train_hr_imgs), batch_size):
            ...
            ## update D
            errD, d_gan, d_code, summary, _ = sess.run(
                [d_loss, d_gan_loss, d_code_loss, merged, d_optim], {
                    t_image: lr_image,
                    t_target_image: hr_image
                })
            loss_writer.add_summary(summary, step)
            ## update G
            errG, mse, vgg, g_gan, g_code, _ = sess.run(
                [g_loss, mse_loss, vgg_loss, g_gan_loss, g_code_loss, g_optim],
                {
                    t_image: lr_image,
                    t_target_image: hr_image
                })
            print(
                "Epoch [%2d/%2d] %4d, d_loss: %.8f(d_gan: %.8f, d_code: %.8f) g_loss: %.8f(mse: %.8f, vgg: %.8f, g_code: %.8f, g_gan: %.8f)"
                % (epoch + 1, num_epoch, step, errD, d_gan, d_code, errG, mse,
                   vgg, g_code, g_gan))
            step = step + 1
        ...
        if (epoch + 1) % 5 == 0 :
            tl.files.save_npz(
                net_g.all_params,
                name=checkpoint_dir + '/full/generator.npz',
                sess=sess)
            tl.files.save_npz(
                net_d.all_params,
                name=checkpoint_dir + '/full/discriminator.npz',
            ...
            tl.files.save_npz(
                net_e.all_params,
                name=checkpoint_dir + '/full/encoder.npz',
                sess=sess)
            tl.files.save_npz(
                net_g.all_params,
                name=checkpoint_dir + '/full/generator%d.npz' % (epoch + 1),
                sess=sess)
            tl.files.save_npz(
                net_d.all_params,
                name=checkpoint_dir + '/full/discriminator%d.npz' % (epoch + 1),
                sess=sess)
            tl.files.save_npz(
                net_c.all_params,
                name=checkpoint_dir + '/full/code_discriminator%d.npz' % (epoch + 1),
                sess=sess)
            tl.files.save_npz(
                net_e.all_params,
                name=checkpoint_dir + '/full/encoder%d.npz' % (epoch + 1),
                sess=sess)
